...
def BlockImages3(loopcount):
    ...
    dirn = '/home/pi/src/bloxl/fadecandy/examples/python/monday'
    ...
    for q in range(loopcount):
        ...
        for i in range(220):
                if i < 100:
                    MorphImages.append(dirn+"/monday0"+str(i)+".png")
                else:
                    MorphImages.append(dirn+"/monday"+str(i)+".png")
        print(MorphImages)
        for picture in range(len(MorphImages)):
            img = Image.open(MorphImages[picture])
            img = img.convert('RGB')
...
def BlockImages4(loopcount):
    ...
    dirn = '/home/pi/src/bloxl/fadecandy/examples/python/monday2'
...
def RunAnimations(loopcount=1, loop_delay=.05):
    ...
    for cnt in range(loopcount):
        ...
        for folder in os.listdir(dirn):
            ...
            sort_images.sort(key=lambda x: x[2])
